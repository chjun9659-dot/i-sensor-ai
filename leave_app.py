...
with btn_col1:
    register_btn = st.button("등록하기", use_container_width=True)
...
with st.form("add_employee_form"):
    ...
    new_total_leave = st.number_input("발생 연차", min_value=0.0, step=0.5, value=15.0)
...
if submit_add_employee:
    ...
    if new_name == "":
        ...
        hire_date = pd.to_datetime(new_hire_date)
        base_year = 2026
        start_date, end_date = calculate_anniversary_period(hire_date.date(), base_year)
        service_years = calculate_service_years(hire_date.date(), start_date)
        ...
        new_row = {
            ...
            "발생 연차": float(new_total_leave),
            "사용 연차": 0.0,
            "잔여 연차": float(new_total_leave),
        }
...
with st.form("edit_employee_form"):
    ...
    edited_total_leave = st.number_input(
        "발생 연차 수정",
        min_value=0.0,
        step=0.5,
        value=float(to_number(edit_employee["발생 연차"]))
    )
...
if submit_edit_employee:
    ...
    if edited_name == "":
        ...
        if edited_name in duplicate_names:
            ...
            hire_date = pd.to_datetime(edited_hire_date)
            base_year = 2026
            start_date, end_date = calculate_anniversary_period(hire_date.date(), base_year)
            service_years = calculate_service_years(hire_date.date(), start_date)

            new_total = float(edited_total_leave)
...
# 월별 통계
# -----------------------------
st.subheader("📅 월별 연차 통계")

stat_col1, stat_col2 = st.columns(2)

with stat_col1:
    stat_year = st.number_input("조회 연도", min_value=2020, max_value=2100, value=2026, step=1)

with stat_col2:
    stat_month = st.selectbox("조회 월", list(range(1, 13)), index=2)
...
styled_df = display_df[show_cols].style.map(style_remaining_leave, subset=["잔여 연차"])
